...
def convert_variant2_predicted_test_to_submission_format(
    ...
    t1_patient_names = [i.split("_")[-1][:-7] for i in
        subfiles(folder_with_predictions, prefix="T1", suffix=".nii.gz", join=False)]
    ...
    merge(
        (folder_for_ensembing0, folder_for_ensembing1), final_predictions_folder, 8, True,
    ...
    apply_postprocessing_to_folder(
        final_predictions_folder, postprocessed_folder,
...
if __name__ == "__main__":
    ...
    root = "datasets/CHAOS/Train_Sets"
    root_test = "datasets/CHAOS/Test_Sets"
    ...
    for p in patients:
        ...
        _ = dicom2nifti.convert_dicom.dicom_series_to_nifti(img_dir, img_outfile, reorient_nifti=False)
        shutil.copy(
            join(output_images, patient_name + "_0000.nii.gz"), join(output_images, patient_name + "_0001.nii.gz")
        )
    ...
    for p in patients:
        ...
        _ = dicom2nifti.convert_dicom.dicom_series_to_nifti(img_dir, img_outfile, reorient_nifti=False)
        shutil.copy(
            join(output_imagesTs, patient_name + "_0000.nii.gz"), join(output_imagesTs, patient_name + "_0001.nii.gz")
        )
