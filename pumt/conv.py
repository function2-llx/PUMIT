...
class InflatableConv3d(nn.Conv3d):
    ...
    def forward(self, x: SpatialTensor):
        ...
        if x.aniso_d > x.num_downsamples:
            ...
            weight = self.weight
        x = x.copy_meta_to(nnf.conv3d(x, weight, self.bias, stride, padding, self.dilation, self.groups))
        return x
...
class AdaptiveConvDownsample(AdaptiveDownsample):
    def __init__(
        ...
        self.conv = conv_t(in_channels, out_channels, kernel_size, stride, bias=bias, d_inflation=d_inflation)
    ...
    def downsample(self, x: SpatialTensor):
        if x.downsample_ready:
            weight = self.conv.weight
            stride = (2, 2, 2)
        else:
            weight = self.conv.weight.sum(dim=1, keepdim=True)
            stride = (1, 2, 2)
        pad = [
            (0, 0) if s == 1
            else (k - 2 >> 1, k - 1 >> 1)
        ...
        return x.copy_meta_to(nnf.conv3d(nnf.pad(x, pad), weight, self.conv.bias, stride))
